...
import { Row, Col, Card, Typography, Tag, Divider, Spin, Empty, Alert } from "antd";
...
const toDate = (iso?: string) => (iso ? iso.slice(0, 10) : ""); //lấy giờ từ giờ nhị phân cắt ra 10
...
export default function Home() {
  ...
  // Danh mục từ dữ liệu (giữ nguyên, không phụ thuộc lọc)
  ...
  // 1) Lọc theo từ khóa (tiêu đề + nội dung)
  ...
  // ⭐ Recent cố định theo toàn bộ posts (không phụ thuộc tìm kiếm/danh mục)
  const recentFeature = posts[posts.length -1];
  const recentRightCol = posts.slice(posts.length-3, posts.length-1);
          ...
          <Row gutter={[24, 24]}>
            <Col xs={24} lg={14}>
              ...
              >
                ...
                <Title level={4} className="title">{recentFeature.title}</Title>
